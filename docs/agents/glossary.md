...
|---|---|---|
| **UBB** | User-Level Budget | A spending cap that applies to a single user (or to a default "everyone without their own cap"). Two flavors in this app: *individual UBB* and *universal UBB*. The letters don't match the long form — the repo was renamed from "ULB" to "UBB" but kept the long form. Treat "UBB" as the canonical abbreviation. |
| **Individual UBB** | — | A per-user cap. The thing this dashboard exists to manage in bulk. |
| **Universal UBB** | — | A single cap that applies to every user who does *not* have an individual UBB set. Acts as the org-wide default. |
...
| Term | Meaning |
|---|---|
| **Pool** | The shared, enterprise-wide bucket of *included* AICs. Sized as `(CB seats × CB allowance) + (CE seats × CE allowance)`. Drained first, by every user, regardless of CC or UBB. See [ai-credit-pool.md](./ai-credit-pool.md). |
...
| **Metered AICs** | Spend *after* the pool is drained — i.e. true overage that lands against a UBB / CC / enterprise budget. See [metered-ai-credits.md](./metered-ai-credits.md). |
| **Pool draw** | Synonym for gross AIC consumption. |
| **Routing precedence** | The order in which a metered AIC charge gets attributed: individual UBB → CC budget → universal UBB → enterprise budget. The first matching scope owns the charge. |
...
| Term | Meaning |
|---|---|
| **Bulk unblock** | One-click action: raise the UBB cap on N selected users so they stop being blocked for the rest of the month. The headline feature. |
...
| **Allocation diagram** | The visual that explains how a $X enterprise budget breaks into CC and UBB sub-budgets. Two display modes: independent bars and rolled-up. |
...
- **"Universal UBB"** is *not* a synonym for "enterprise budget." A
  universal UBB caps individuals (it's still a UBB); the enterprise
  budget caps everything.
- **"Budget"** is overloaded. It can mean any of: individual UBB cap,
  universal UBB cap, CC budget, enterprise budget. When writing code or
