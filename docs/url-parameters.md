...
- **Scheme-less enterprise URL** (any trusted host, including GHE.com):
  - `?ent=github.com/enterprises/acme` → `https://github.com/enterprises/acme`
  - `?ent=customer.ghe.com/enterprises/acme` → `https://customer.ghe.com/enterprises/acme`

The second shape is just the address-bar URL with `https://` stripped
off, so admins can copy from their browser and paste straight in.
Trailing path segments (`/settings`, `/billing`, etc.) are ignored.

The older full-URL form (`?ent=https://customer.ghe.com/enterprises/acme`)
is still accepted as a silent fallback for links generated by earlier
builds. Invalid values (random garbage, non-GitHub hosts, slugs with
disallowed characters) are silently ignored. The form stays usable
and falls back to the `VITE_DEV_ENTERPRISE_URL` default if one is set,
or empty otherwise.
